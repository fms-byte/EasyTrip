      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {photos.map((photo, index) => (
          <div key={index} className="relative group">
            ...
            />
            <div
              className="absolute bottom-0 left-0 w-full bg-black bg-opacity-50 text-white text-center p-2 opacity-0 group-hover:opacity-100 transition"
            >
              {photo.feature || "N/A"}
            </div>
            <div className="mt-2">
              <h3 className="text-lg font-semibold">{photo.feature.slice(0,20) || "N/A"}</h3>
              <p className="text-sm text-gray-500">Uploaded by: {photo.name}</p>
            </div>
          </div>
        ))}
      </div>
