...
export default function Homepage() {
  ...
  const handleGeneratePlan = async (inputText: string) => {
    ...
    const tripExtract = `http://localhost:3000/api/tripPlan/extract?text=${inputText}`;
    console.log("Trip Prompt:", tripExtract);
    // API Call
    try {
      const response = await axios.get(tripExtract, {
        headers: {
          "Content-Type": "application/json",
        },
      });
